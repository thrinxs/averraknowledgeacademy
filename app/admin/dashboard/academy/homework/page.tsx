...
export default async function AdminHomeworkPage() {
  ...
  const avgScore = submitted.length > 0
    ? Math.round(submitted.reduce((sum, h) => sum + ((h.score || 0) / (h.max_score || 1)) * 100, 0) / submitted.length)
    : 0

  return (
    <div className="p-6 md:p-10">
      <div className="mb-8">
        <h1 className="text-2xl md:text-3xl font-bold mb-2" style={{ color: '#062850' }}>Homework Overview</h1>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100">
          <h2 className="font-bold text-lg" style={{ color: '#062850' }}>All Homework Today</h2>
        </div>
        {(!homeworks || homeworks.length === 0) ? (
          <div className="p-12 text-center">
            <FileText className="w-10 h-10 mx-auto mb-3" style={{ color: '#D1D5DB' }} />
            <p className="text-gray-500">No homework assigned today yet</p>
          </div>
        ) : (
              ...
              return (
                <div key={hw.id} className="px-6 py-4 flex items-center justify-between gap-4">
                  <div>
                    ...
                    <p className="text-xs text-gray-500">{hw.subject_code} — {hw.topic_name}</p>
