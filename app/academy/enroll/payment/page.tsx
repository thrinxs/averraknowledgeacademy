  ...
  useEffect(() => {
    ...
    if (!enrollmentId) {
      ...
      if (id) {
        ...
        window.location.replace(`/academy/enroll/payment?enrollment_id=${id}&currency=${cur}&amount=${amt}`)
      }
    }
  }, [enrollmentId, preNgn])
        ...
        {isNGN && (
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden mb-6">
            <div className="px-8 py-5" style={{ backgroundColor: '#062850' }}>
              ...
              <p className="text-blue-300 text-sm mt-0.5">Powered by Paystack - Visa, Mastercard and Apple Pay</p>
            </div>
          ...
          <>
            {!method && (
              <div className="space-y-3 mb-6">
                ...
                <button onClick={() => setMethod('paystack')} className="w-full bg-white rounded-2xl border-2 border-gray-100 p-5 flex items-center gap-4 text-left hover:border-[#497296] transition-all hover:shadow-md group">
                  <span className="text-3xl">&#128179;</span>
                  <div className="flex-1">
                    <p className="font-bold text-sm" style={{ color: '#062850' }}>Pay by Card</p>
                    <p className="text-xs text-gray-500 mt-0.5">Visa, Mastercard, Apple Pay - charged in NGN at live rate</p>
                    {ngnAmount > 0 && <p className="text-xs font-semibold mt-1" style={{ color: '#497296' }}>Approx. charge: ₦{ngnAmount.toLocaleString()} + Paystack fee</p>}
                  </div>
                  <span className="text-xs font-bold text-white px-2 py-1 rounded-full" style={{ backgroundColor: '#16A34A' }}>Instant</span>
                </button>
                <button onClick={() => setMethod('gbp')} className="w-full bg-white rounded-2xl border-2 border-gray-100 p-5 flex items-center gap-4 text-left hover:border-[#497296] transition-all hover:shadow-md group">
                  <span className="text-3xl">&#127468;&#127463;</span>
                  <div className="flex-1">
                    <p className="font-bold text-sm" style={{ color: '#062850' }}>GBP Bank Transfer</p>
                    <p className="text-xs text-gray-500 mt-0.5">Faster Payment (FPS) - no processing fee</p>
                  </div>
                  <span className="text-xs font-bold text-white px-2 py-1 rounded-full" style={{ backgroundColor: '#062850' }}>GBP</span>
                </button>
                <button onClick={() => setMethod('eur')} className="w-full bg-white rounded-2xl border-2 border-gray-100 p-5 flex items-center gap-4 text-left hover:border-[#497296] transition-all hover:shadow-md group">
                  <span className="text-3xl">&#127466;&#127482;</span>
                  <div className="flex-1">
                    <p className="font-bold text-sm" style={{ color: '#062850' }}>EUR Bank Transfer</p>
                    <p className="text-xs text-gray-500 mt-0.5">SEPA and SEPA Instant - no processing fee</p>
                  </div>
                  <span className="text-xs font-bold text-white px-2 py-1 rounded-full" style={{ backgroundColor: '#1D4469' }}>EUR</span>
                </button>
              </div>
            )}
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden mb-6">
                ...
                <div className="px-8 py-6 space-y-4">
                  <div className="p-4 rounded-xl text-sm" style={{ backgroundColor: '#FFF8F0', borderLeft: '4px solid #F59E0B' }}>
                    <p className="font-semibold text-amber-800 mb-1">Currency Note</p>
                    <p className="text-amber-700 text-xs">Your card will be charged in NGN (Nigerian Naira). Your bank converts from {currency} at their live rate. {ngnAmount > 0 && <>Approximate NGN amount: ₦{ngnAmount.toLocaleString()}. </>}Paystack will confirm the exact amount before you pay.</p>
                  </div>
                  ...
                  <Button onClick={payWithPaystack} disabled={loadingPS || paystackNGN === 0} className="w-full py-4 text-white font-semibold rounded-xl text-base" style={{ backgroundColor: '#497296' }}>
                    {loadingPS ? <><Loader2 className="w-4 h-4 animate-spin mr-2 inline" />Connecting...</> : paystackNGN === 0 ? <><Loader2 className="w-4 h-4 animate-spin mr-2 inline" />Loading rate...</> : psLabel}
                  </Button>
                </div>
              </div>
            )}
            {method === 'gbp' && (
              <BankCard title="GBP Bank Transfer" subtitle="Faster Payment (FPS) - No processing fee" flag="&#127468;&#127463;" details={GBP_DETAILS} reference={reference} setReference={setReference} copiedField={copiedField} copy={copy} onBack={() => setMethod(null)} note="Please send via Faster Payment (FPS) for instant transfer" amount={formatted} />
            )}
            {method === 'eur' && (
              <BankCard title="EUR Bank Transfer" subtitle="SEPA and SEPA Instant - No processing fee" flag="&#127466;&#127482;" details={EUR_DETAILS} reference={reference} setReference={setReference} copiedField={copiedField} copy={copy} onBack={() => setMethod(null)} note="Please send via SEPA Instant for fastest processing" amount={formatted} />
            )}
          </>
        )}
        ...
        <div className="rounded-2xl p-6 border-2" style={{ borderColor: '#497296', backgroundColor: '#F0F6FB' }}>
          ...
          <div className="mt-4 p-4 rounded-xl border-2 border-dashed" style={{ borderColor: '#497296' }}>
            ...
            <div className="flex items-center gap-2">
              <input type="text" value={reference} onChange={(e) => setReference(e.target.value)} className="flex-1 font-bold font-mono text-center px-3 py-2 rounded-lg border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#497296]" style={{ color: '#062850' }} />
          ...
          <div className="mt-4 p-3 rounded-xl text-sm text-center font-semibold" style={{ backgroundColor: '#EBF4FF', color: '#062850' }}>{note}</div>
        </div>
      </div>
    </div>
  )
