...
create table if not exists academy_countries (
  ...
  curriculum_authority text not null
);
...
create table if not exists local_curricula (
  id uuid primary key default gen_random_uuid(),
  country_code text references academy_countries(country_code),
  year_group_code text not null,
  subject_code text not null,
...
create table if not exists averra_super_curriculum (
  id uuid primary key default gen_random_uuid(),
  country_code text references academy_countries(country_code),
  year_group_code text not null,
  subject_code text not null,
  ...
  unique_additions jsonb not null default '{}',
  competencies jsonb not null default '{}',
  created_at timestamptz default now(),
  updated_at timestamptz default now(),
  unique(country_code, year_group_code, subject_code)
...
-- RLS
alter table academy_countries enable row level security;
alter table year_group_equivalencies enable row level security;
alter table country_subjects enable row level security;
alter table local_curricula enable row level security;
alter table averra_super_curriculum enable row level security;

create policy "Public read academy_countries"
  on academy_countries for select using (true);
...
create policy "Public read local_curricula"
  on local_curricula for select using (true);
create policy "Public read averra_super_curriculum"
  on averra_super_curriculum for select using (true);

-- ─────────────────────────────────────────────
-- STEP 2: SEED COUNTRIES
-- ─────────────────────────────────────────────

insert into academy_countries (country_code, country_name, flag, curriculum_name, curriculum_authority) values
('NG', 'Nigeria',      '🇳🇬', 'Nigerian National Curriculum',          'NERDC / WAEC / NECO'),
('GB', 'England',      '🇬🇧', 'England National Curriculum',           'DfE / Ofqual'),
('JP', 'Japan',        '🇯🇵', 'Japanese Course of Study',              'MEXT'),
('EE', 'Estonia',      '🇪🇪', 'Estonian National Curriculum',          'Ministry of Education'),
('CA', 'Canada',       '🇨🇦', 'Provincial Curricula (Ontario base)',   'Provincial Ministries'),
('SG', 'Singapore',    '🇸🇬', 'Singapore Curriculum Framework',        'MOE Singapore'),
('FI', 'Finland',      '🇫🇮', 'Finnish National Core Curriculum',      'Finnish National Agency for Education'),
('GH', 'Ghana',        '🇬🇭', 'Ghanaian National Curriculum',          'GES / WAEC'),
('ZA', 'South Africa', '🇿🇦', 'South African CAPS',                   'DBE / Umalusi'),
('KE', 'Kenya',        '🇰🇪', 'Kenyan CBC Curriculum',                 'KNEC / KICD'),
('AU', 'Australia',    '🇦🇺', 'Australian Curriculum',                 'ACARA'),
('IE', 'Ireland',      '🇮🇪', 'Irish National Curriculum',             'NCCA / SEC'),
('IN', 'India',        '🇮🇳', 'Indian NCERT Curriculum',               'NCERT / CBSE')
on conflict (country_code) do nothing;

-- ─────────────────────────────────────────────
-- STEP 3: SEED YEAR GROUPS
-- ─────────────────────────────────────────────
...
-- ─────────────────────────────────────────────
-- STEP 4: SEED COUNTRY SUBJECTS
-- averra_teaches = true if covered by our 18 subject codes
-- ─────────────────────────────────────────────
...
cross join (values
  ...
  ('NG_BST','Basic Science & Technology','compulsory',true,'BTECH'),
  ...
  ('NG_SCI','Science','compulsory',true,'SCI'),
...
cross join (values
  ...
  ('NG_NHC','History','elective',true,'HIST')
) as s(subject_code, subject_name, subject_type, averra_teaches, averra_subject_code)
...
cross join (values
  ...
  ('ZA_FAL','First Additional Language','compulsory',true,'ENG'),
  ...
  ('ZA_CHEM','Physical Sciences (Chem)','elective',true,'CHEM'),
