...
export default function PrincipalTopBar({ fullName, email, avatarUrl, onMobileMenuToggle }: Props) {
  ...
  return (
    <header className="fixed top-0 right-0 left-0 lg:left-64 z-30 h-16 flex items-center justify-between px-4 md:px-6 border-b border-gray-100 bg-white">
      <button onClick={onMobileMenuToggle} className="lg:hidden p-2 rounded-xl text-gray-500 hover:bg-gray-100 transition-colors">
        <Menu className="w-5 h-5" />
      </button>
      <div className="flex-1 lg:flex-none px-2 lg:px-0">
        <span className="lg:hidden text-sm font-bold" style={{ color: '#062850' }}>Averra Principal Portal</span>
        <span className="hidden lg:block text-sm font-bold" style={{ color: '#062850' }}>Principal Dashboard</span>
      </div>
      <div className="relative" ref={dropdownRef}>
        <button onClick={() => setDropdownOpen(p => !p)}
          ...
          <div className="hidden md:block text-left">
            ...
            <p className="text-xs leading-tight" style={{ color: '#497296' }}>Principal</p>
          </div>
        ...
        {dropdownOpen && (
          <div className="absolute right-0 top-full mt-2 w-56 bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden z-50">
            <div className="px-4 py-3 border-b border-gray-100" style={{ backgroundColor: '#F0F6FB' }}>
              <p className="text-xs font-bold uppercase tracking-wide mb-0.5" style={{ color: '#497296' }}>Principal</p>
              <p className="text-sm font-bold truncate" style={{ color: '#062850' }}>{fullName}</p>
              <p className="text-xs text-gray-400 truncate">{email}</p>
            </div>
            <div className="py-1">
              <Link href="/principal/dashboard/profile" onClick={() => setDropdownOpen(false)}
                className="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-600 hover:bg-gray-50 transition-colors">
                <User className="w-4 h-4 text-gray-400" /> My Profile
              </Link>
              <Link href="/principal/dashboard/settings" onClick={() => setDropdownOpen(false)}
                className="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-600 hover:bg-gray-50 transition-colors">
              ...
              <div className="border-t border-gray-100 mt-1 pt-1">
                ...
                <button onClick={async () => { await supabase.auth.signOut(); window.location.href = '/auth/staff-login' }}
