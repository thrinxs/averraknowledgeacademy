...
import Step2AcademicBackground from
  './steps/Step2AcademicBackground'
import Step3CountryPreferences from
  './steps/Step3CountryPreferences'
import Step4CoursePreferences from
  './steps/Step4CoursePreferences'
import Step5AccountCreation from
  './steps/Step5AccountCreation'
...
export default function FormShell() {
  ...
  const [savedIndicator, setSavedIndicator] =
    useState(false)
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        setFormData({
          ...parsed,
          package:
            packageParam || parsed.package || '',
          promoCode: parsed.promoCode || '',
          promoDiscount:
            parsed.promoDiscount || null,
          paymentTiming:
            parsed.paymentTiming || 'now',
        })
      } catch {
        // ignore corrupt data
      }
    ...
    // Re-check auth when user returns to this page
    // This catches the case where user went to login
    // and came back via history.back()
    ...
    // Listen for Supabase auth state changes in
    // real time — catches login from overlay
    const {
      data: { subscription },
  ...
  const validateStep = (step: number): boolean => {
    ...
    if (step === 4) {
      ...
      if (!formData.fieldAbroad.trim())
        errors.fieldAbroad =
          'Field of study abroad is required'
      if (!formData.reasonForStudying.trim())
        errors.reasonForStudying =
          'Please tell us your reason for studying abroad'
    }
    ...
    try {
      if (currentUser) {
        ...
        const response = await fetch(
          '/api/scholarship/save-preferences',
          {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({
              ...
              institutionAccredited:
                formData.institutionAccredited,
              graduationYear:
  ...
  // All hooks above this line — safe to return early
  ...
  return (
    <div>
      ...
      border border-gray-100 p-8 md:p-10">

        {currentStep === 1 && (
          <Step1BasicInfo {...stepProps} />
        )}
        {currentStep === 2 && (
          <Step2AcademicBackground {...stepProps} />
        )}
        {currentStep === 3 && (
          <Step3CountryPreferences {...stepProps} />
        )}
        {currentStep === 4 && (
          <Step4CoursePreferences {...stepProps} />
        )}
        {currentStep === 5 && (
